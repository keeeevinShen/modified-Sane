 ...
 */
 ...
 usb_ScanBegin( Plustek_Device *dev, SANE_Bool auto_park )
 {
	 ...
	 #if 1
	 /* --- HANDHELD MODE MODIFICATION START --- */
	 if ( m_pParam->bCalibration == PARAM_Scan ) {
 
		 /* 1. Move Sensor to Middle 
		  * usb_ModuleMove changes registers to "Fast Feed" mode (0x08, 0x48, 0x49, etc.)
		  * This is why your previous attempt resulted in black images. 
		  */
		 DBG( _DBG_INFO, ">>> HANDHELD MODE: Repositioning Sensor to Middle (2100) <<<\n" );
		 
		 if (!usb_ModuleMove(dev, MOVE_Forward, 2100UL)) {
			  DBG( _DBG_ERROR, "Failed to move sensor to middle position for Handheld Mode\n" );
			  return SANE_FALSE;
		 }
 
		 /* 2. RESTORE SCAN REGISTERS 
		  * We must revert the changes made by usb_ModuleMove. 
		  * usb_SetScanParameters re-calculates and writes the correct MCLK, PWM, 
		  * and DPI settings for the actual scan.
		  */
		 DBG( _DBG_INFO, ">>> HANDHELD MODE: Restoring Scan Parameters <<<\n" );
		 if( !usb_SetScanParameters( dev, m_pParam )) {
			 DBG( _DBG_ERROR, "Failed to restore scan parameters after move\n" );
			 return SANE_FALSE;
		 }
 
		 /* 3. Disable Motor Power 
		  * Now that registers are correct, we cut power to the coils.
		  * We do this AFTER usb_SetScanParameters because that function enables the motor.
		  */
		 DBG( _DBG_INFO, ">>> HANDHELD MODE: Disabling Motor Power <<<\n" );
		 
		 /* Mask out bit 4 (Motor Enable) from the current shadow register */
		 if (!usbio_WriteReg(dev->fd, 0x45, regs[0x45] & ~0x10)) {
			 DBG(_DBG_ERROR, "Failed to disable motor power (Reg 0x45)\n");
			 return SANE_FALSE;
		 }
	 }
	 /* --- HANDHELD MODE MODIFICATION END --- */
	 #endif
