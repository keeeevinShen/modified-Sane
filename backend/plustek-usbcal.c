...
#define MY_MIDDLE_Y 2100 
#define MY_BEEP_STEP  100
	...
	if( !skip_fine ) {
		...
		for( idx = 1; idx < idx_end; idx++ ) {
			...
			if( !usb_IsSheetFedDevice(dev)) {
				...
				/* Short pause so it's clearly parked in middle */
				DBG(_DBG_INFO2, "Arrived at middle; sleeping 3 seconds\n");
				sleep(3);
			
				/* Wiggle forward/backward to make an audible cue */
				DBG(_DBG_INFO2, "Doing wiggle beep at middle\n");
				if( !usb_ModuleMove(dev, MOVE_Forward, (u_long)MY_BEEP_STEP)) {
					return SANE_FALSE;
				}
				/* If your driver has MOVE_Backward (or similar), use it here.
				   If the symbol is named differently, adjust accordingly. */
				if( !usb_ModuleMove(dev, MOVE_Backward, (u_long)MY_BEEP_STEP)) {
					return SANE_FALSE;
				}
			
				/* Now give user time to hear it and open lid */
				DBG(_DBG_INFO2, "Waiting 5 seconds for user to open lid/place white target\n");
				sleep(5);
			}
