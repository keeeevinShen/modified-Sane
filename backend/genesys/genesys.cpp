...
static void init_options(Genesys_Scanner* s)
{
  ...
  // Add this block inside the init_options() function in genesys.cpp

    s->opt[OPT_FORCE_RETURN_HOME].name = "force-return-home";
    s->opt[OPT_FORCE_RETURN_HOME].title = SANE_I18N ("Force Return to Home");
    s->opt[OPT_FORCE_RETURN_HOME].desc = SANE_I18N ("Immediately moves the scanner assembly back to the home position.");
    s->opt[OPT_FORCE_RETURN_HOME].type = SANE_TYPE_BUTTON;
    s->opt[OPT_FORCE_RETURN_HOME].unit = SANE_UNIT_NONE;
    s->opt[OPT_FORCE_RETURN_HOME].cap = SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT | SANE_CAP_ADVANCED;
...
//add a new case OPT_FORCE_RETURN_HOME
        case OPT_FORCE_RETURN_HOME: {
            // This is the bridge to the basis operation.
            // It calls the function we identified in command_set.h
            dev->cmd_set->move_back_home(dev, true);
            break;
        }
